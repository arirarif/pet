...
async function fetchPetsByCategory(categoryName) {
    ...
    try {
        ...
        if (pets.length === 0) {
            displayNoDataMessage("Sorry, birds data is missing.");
        } else {
            displayPets(pets);
        }
    } catch (error) {
...
async function fetchCategories() {
    try {
        ...
        data.categories.forEach((category) => {
            ...
            categoryDiv.className = "category-card border border-gray-200 rounded-full p-6 bg-white shadow-md hover:shadow-lg hover:bg-[#e6f1f2] hover:border-[#0E7A811A] transition-all";
